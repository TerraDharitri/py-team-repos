...
def test_user_signer_from_pem_file():
    ...
    assert converter.pubkey_to_bech32(pubkey.buffer) == "drt1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssey5egf"

    pubkey = UserSigner.from_pem_file(Path("./dharitri_sdk_wallet/testdata/bob.pem"), 0).get_pubkey()
    assert converter.pubkey_to_bech32(pubkey.buffer) == "drt1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruqlqde3c"

    pubkey = UserSigner.from_pem_file(Path("./dharitri_sdk_wallet/testdata/carol.pem"), 0).get_pubkey()
    assert converter.pubkey_to_bech32(pubkey.buffer) == "drt1k2s324ww2g0yj38qn2ch2jwctdy8mnfxep94q9arncc6xecg3xaq889n6e"
...
def test_sign_transaction():
    ...
    verifier = UserVerifier.from_address(Address.from_bech32("drt1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssey5egf"))
...
def test_load_secret_key_but_without_kind_field():
    ...
    actual_address = Address(secret_key.generate_public_key().buffer, "drt").bech32()
...
def test_load_secret_key_with_mnemonic():
    ...
    assert UserWallet.load_secret_key(keystore_path, "password", 0).generate_public_key().to_address("drt").bech32() == "drt1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssey5egf"
    assert UserWallet.load_secret_key(keystore_path, "password", 1).generate_public_key().to_address("drt").bech32() == "drt1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruqlqde3c"
    assert UserWallet.load_secret_key(keystore_path, "password", 2).generate_public_key().to_address("drt").bech32() == "drt1k2s324ww2g0yj38qn2ch2jwctdy8mnfxep94q9arncc6xecg3xaq889n6e"
